...
class dwImagick {
	...
	public function __construct($fileName=''){
		...
		if($fileName != ''){
			$this->srcImg = $this->newImagick($fileName);
		}
		if($this->srcImg != null){
    ...
    public function newImagick($fileName=""){
        if( empty($fileName) ) return new Imagick();
        
        @$getImaSize = getimagesize($fileName);
        if (false === $getImaSize) return false;
        
        list($width, $height, $type, $attr) = $getImaSize;
        $imageTypes = array(
            IMAGETYPE_GIF, IMAGETYPE_JPEG, IMAGETYPE_PNG, 
            IMAGETYPE_BMP, IMAGETYPE_ICO,
        );   
        if ( !in_array($type, $imageTypes) ) return false;
	...
	*/
	public function water($water=null, $sourceImg=null) {
		if($water != null){
	    	...
	    	$water = $this->newImagick($water);
		}elseif($this->waterImg){
			$water = $this->newImagick($this->waterImg);
		}
		if(!isset($water) || $water == null)return;
		if($sourceImg != null){
			$image = $this->newImagick($sourceImg);
		}elseif($this->srcImg){
			$image = $this->srcImg;
		}
	...
	*/
	...
	public function setWaterImg($water){
		...
		//	$this->waterImg = $this->newImagick($water);
	}
