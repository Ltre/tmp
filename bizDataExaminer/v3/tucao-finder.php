...
class Finder {
    ...
    function initDB($source, $batch){
        ...
        $this->sqlite->query("create table if not exists tucao(
    ...
    function del(){
        ...
        foreach ($list as $v) {
            $ret = $this->http->post(ltredc($d), ['dels' => $v['id'], 'domain' => $v['domain'], 'sign' => md5("tucao{$v['id']}")], 55);
            $log = "id: {$v['id']}, domain: {$v['domain']}, sign: ".md5("tucao{$v['id']}").",ret: {$ret}";
            $this->log('del', $log);
            echo $log."\n";
        }
    }
    ...
    function test(){
        print_r($this->sqlite->query("select * from tucao limit 10"));
    }
...
if ($func) {    
    $finder = new Finder('tucao', $batch);
    call_user_func_array([$finder, $func], $params);
}

//example: php tucao-finder.php 20180528 scan 0 99999999 "tu.duowan.com"
//example: php tucao-finder.php 20180528 sql "select count(1),domain from tucao group by domain"
//example: php tucao-finder.php 20180528 del
