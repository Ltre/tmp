...
class Finder {
    ...
    function initDB($source, $batch){
        ...
        $this->sqlite->query("create table if not exists bbsuser(
    ...
    function del(){
        ...
        $list = $this->sqlite->query("select * from bbsuser");
        foreach ($list as $v) {
            $ret = $this->http->post(ltredc($d), ['id' => $v['id'], 'sign' => md5("bbs{$v['id']}")], 55);
    ...
    function test(){
        print_r($this->sqlite->query("select * from bbsuser limit 10"));
    }
...
if ($func) {    
    $finder = new Finder('bbsuser', $batch);
