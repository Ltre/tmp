...
class Finder {
    ...
    function del(){
        ...
        $list = $this->sqlite->query("select id,tb_id from bbspost where keywords like '%董存瑞%' or keywords like '%邱少云%' or keywords like '%黄继光%' or keywords like '%黄河大合唱%' or keywords like '%李伯伯要当红军%' or keywords like '%长征组歌%' or keywords like '%闪闪的红星%' or keywords like '%铁道游击队%' or keywords like '%热血雷锋侠%' or keywords like '%堵枪眼%' or keywords like '%炸碉堡%' or keywords like '%狼牙山五壮士%' or keywords like '%铁道游击队%' or keywords like '%刘胡兰%' or keywords like '%革命%' or keywords like '%烈士%' or keywords like '%雷锋%' or keywords like '%抗日%' or keywords like '%先烈%' or keywords like '%革命%' or keywords like '%李大钊%' or keywords like '%邓中夏%' or keywords like '%蔡和森%' or keywords like '%杨匏安%' or keywords like '%恽代英%' or keywords like '%瞿秋白%' or keywords like '%赵世炎%' or keywords like '%罗亦农%' or keywords like '%陈潭秋%' or keywords like '%左权%' or keywords like '%叶挺%' or keywords like '%鲁迅%' or keywords like '%李大钊%' or keywords like '%陈独秀%' or keywords like '%毛泽民%' or keywords like '%叶挺%' or keywords like '%项英%' or keywords like '%王克勤%' or keywords like '%狼牙山五壮士%' or keywords like '%白求恩%' or keywords like '%毛岸英%' ");
        foreach ($list as $v) {
            $ret = $this->http->post(ltredc($d), ['pids' => $v['id'], 'tb_id' => $v['tb_id'], 'sign' => md5("bbs{$v['id']}")], 55);
            $log = "id: {$v['id']}, tb_id: {$v['tb_id']}, sign: ".md5("bbs{$v['id']}").",ret: {$ret}";
            $this->log('del', $log);
            echo $log."\n";
        }
    }
